...
const Entity = class {
    ...
    /**
     *  Check if the entiy is stored inside the root object store.
     *  @return boolean
     */
    get isStored() {

        // if the entity has no root, then it was never attached to a root
        if (!this.root) return false;

        // check if the root has the entity stored
        return !!this.root.fetch(this.constructor, this.id);
    }

    /**
     *  Store the entity in the current root.
     *  @return Entity
     */
    store() {

        // tell the root to store the entity.
